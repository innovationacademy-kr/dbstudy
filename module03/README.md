...
* ##### 여러 트랜잭션들은 동시에 S_LOCK을 취득하여, 레코드를 동시에 읽어내는 것은 가능하다.
* ##### X_LOCK은 쓰기 연산을 수행하려는 레코드에 대해 단 하나의 트랜잭션만 취득할 수 있다. 따라서 서로 다른 트랜잭션의 S_LOCK / X_LOCK , X_LOCK / X_LOCK과 같은 중복 잠금은 불가능하다.
...
##### 5. Lock Escalation으로 올라간 잠금 수준은 언제 떨어지고 어떻게 모니터링 되는가? 특히 계속해서 많은 트랜잭션이 들어오면 끝까지 잠금 수준의 회복이 안 되는가?
...
##### 6. Num blocked-holders에서 상위 객체란 무엇을 의미하고, 상위 객체에 의해 차단되는 상황이란 무엇인지 궁금함 (167p)
...
##### 7. 실제 서비스에서 느린 질의가 탐지되는 경우도 있는가? 예로 어떤 것이 있는지 궁금함
...
##### 9. Vacuum이 Undo Segment 보다 어떤 장점이 있어서 Cubrid는 이를 이용하는 것인가?
...
##### 10. MVCC에서 이전 데이터를 제거하고 점유된 공간을 재사용하기 위해 자동으로 회수(Cleanup)하는 시스템에 대해서 어떤 기준으로 이뤄지는가?
